...
type FrameStream = Pin<Box<dyn Stream<Item = Result<Frame<Bytes>, ProxyHttpError>> + Send + Sync + 'static>>;
...
impl Body {
  ...
  pub fn stream<S: Stream<Item = Result<Frame<Bytes>, ProxyHttpError>> + Send + Sync + 'static>(stream: S) -> Self {
    Self {
...
pub fn map_request_body<S, T>(request: hyper::Request<S>, f: impl FnOnce(S) -> T) -> hyper::Request<T> {
...
mod test {
  ...
  macro_rules! assert_stream_collect {
    ($body:ident, $expected:expr) => {{
      let mut body = $body;
      let mut buf = Vec::new();
      while let Some(item) = futures::StreamExt::next(&mut body).await.transpose().unwrap() {
  ...
  async fn channel() {
    ...
    tokio::spawn(async move {
      sender.send(Ok(Frame::data(Bytes::from_static(b"hello")))).await.unwrap();
      sender.send(Ok(Frame::data(Bytes::from_static(b" ")))).await.unwrap();
      sender.send(Ok(Frame::data(Bytes::from_static(b"world")))).await.unwrap();
      sender.send(Ok(Frame::trailers(hyper::HeaderMap::new()))).await.unwrap();
    });
